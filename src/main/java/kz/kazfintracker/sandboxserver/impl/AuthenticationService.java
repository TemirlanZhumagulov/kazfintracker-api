...
public class AuthenticationService {
    ...
    private void validatePassword(String password) {
        ...
        if (!Pattern.compile("^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&+=])(?=\\S+$).{8,20}$").matcher(password).matches()) {
            throw new RuntimeException("Should be at least one digit, one upper case, one special character, from 8 to 20 characters and no white spaces.");
        }
